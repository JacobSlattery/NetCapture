...
<div
  bind:this={menuEl}
  class="fixed z-[200] min-w-[200px] py-1
         bg-[var(--nc-surface-1)] border border-[var(--nc-border)] rounded shadow-2xl text-xs"
  {style}
...
>
  {#each items as item}
    {#if 'separator' in item}
      <div class="my-1 border-t border-[var(--nc-border-1)]"></div>
    {:else}
      <button
      ...
      >
        ...
        {#if item.sub}
          <span class="font-mono text-[10px] text-[var(--nc-fg-4)] truncate">{item.sub}</span>
        {/if}
      </button>
    {/if}
  {/each}
